...
module.exports = function(packageName, nodeModulesPath, ui) {
  ...
  // console.log('pathToPackageDefinition',pathToPackageDefinition);

  var packageBasePath = path.normalize(path.join(pathToPackageDefinition, '../..'));
  // console.log(packageBasePath);
  ...
  // try {
  // } catch(err) {
  //   pkg = require(path.join(lookupPackage(packageName), 'package.json'));
  // }

  // try {
  //   fs.statSync(pathToPackageDefinition).isFile();
  // } catch (err) {
  //   ui.writeError('[ember-d3] "' + packageName + '" is not installed as a dependency in your project, cannot determine sub-modules to load.')
  //   return [];
  // }
  ...
  return dependencyNames.map(function(packageName) {
    ...
    var packagePath = pickPath([
      path.join(packageBasePath, packageName),
